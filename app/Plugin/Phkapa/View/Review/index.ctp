        ...
        <table cellpadding="0" cellspacing="0" >
            ...
            foreach ($tickets as $ticket):
                ...
                <tr<?php echo $class; ?>>
                    ...
                    <!--td><?php //echo h($ticket['Category']['name']);  ?></td-->
                    <td><?php echo h($ticket['Activity']['name']); ?></td>
                    <td><?php echo h($ticket['Ticket']['product']);  ?></td>
                    <td><?php
        echo $this->Text->truncate(
                $this->Text->autoParagraph(h($ticket['Ticket']['description']) . $this->Text->autoParagraph($ticket['Ticket']['review_notes'])), 60, array(
            'ellipsis' => '...',
            'exact' => false
        ));
                ?>&nbsp;</td>
                    ...
                    <td class="actions">
                        <?php
                        if ($sendOk) {
                            echo $this->Html->link(__d('phkapa', 'Plan'), array('action' => 'send', $ticket['Ticket']['id']), array('confirm' => __d('phkapa', 'Are you sure you want to send # %s?', $ticket['Ticket']['id'])));
                            echo ' | ';
                        }
                        ?>
